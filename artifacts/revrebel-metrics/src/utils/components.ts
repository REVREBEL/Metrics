...
async function getFileContent(file: NonNullable<RegistryItem['files']>[number]) {
  try {
    const response = await fetch(
      `/api/get-file-content?path=${encodeURIComponent(file.path)}`
    )
...
export const getCachedComponentItem = async (name: string) => {
  return await getComponentItem(name)
}

export const getCachedFileTree = (files: Array<{ path: string; target?: string }>) => {
  if (!files) {
    return null
  }
