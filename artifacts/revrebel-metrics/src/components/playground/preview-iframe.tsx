...
function collectActiveThemeVars(): string {
  ...
  for (let i = 0; i < rootStyle.length; i++) {
    ...
    if (THEME_TOKEN_PREFIXES.some((prefix) => name.startsWith(prefix))) {
      names.push(name);
    }
  }
...
export function PreviewIframe({
  ...
  const html = useMemo(
    () => generateIframeHTML(initialTheme),
    [initialTheme],
  );
  ...
  useEffect(() => {
    ...
    if (!iframe?.contentWindow || !iframeReady) return;
    iframe.contentWindow.postMessage(
      { type: "theme-vars", cssText: collectActiveThemeVars() },
      "*",
    );
  }, [theme, iframeReady]);
