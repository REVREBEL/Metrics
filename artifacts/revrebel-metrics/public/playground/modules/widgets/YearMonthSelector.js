...
// src/widgets/YearMonthSelector/index.tsx
...
function YearMonthSelector({
  ...
  const [availableYears, setAvailableYears] = useState2([]);
  const [monthsByYear, setMonthsByYear] = useState2({});
  const [selectedYear, setSelectedYear] = useState2("");
  const [selectedMonth, setSelectedMonth] = useState2("");
    ...
    async function loadDates() {
      ...
      try {
        ...
        results.forEach((row) => {
          ...
          const y2 = row.year.toString();
          const md = new Date(2e3, row.month - 1, 1);
          const mStr = md.toLocaleString("default", { month: "short" });
          yearsSet.add(y2);
          if (!monthsMap[y2]) monthsMap[y2] = [];
          monthsMap[y2].push(mStr);
        });
  ...
  if (isInitializing || isLoading) {
    return /* @__PURE__ */ jsx4(Skeleton, { className: "h-10 w-full max-w-[600px] rounded-md" });
  }
  if (error) {
    return /* @__PURE__ */ jsx4("div", { className: "text-destructive text-sm font-medium", children: "Failed to load date selector." });
  }
  return /* @__PURE__ */ jsxs("div", { className: "flex flex-row items-center gap-2 w-fit", children: [
    /* @__PURE__ */ jsx4(
      ToggleGroup,
      {
        type: "single",
        ...
        children: availableYears.map((year) => /* @__PURE__ */ jsx4(
          ToggleGroupItem,
          {
            value: year,
            className: "text-sm font-medium h-8 px-5 rounded-sm data-[state=on]:bg-background data-[state=on]:text-foreground data-[state=on]:shadow-sm text-muted-foreground transition-all",
            children: year
          },
          year
        ))
    ...
    /* @__PURE__ */ jsx4(
      ToggleGroup,
      {
        ...
        className: "bg-muted p-1 rounded-sm flex items-center gap-1",
        children: monthsByYear[selectedYear]?.map((month) => /* @__PURE__ */ jsx4(
          ToggleGroupItem,
          {
            value: month,
            className: "text-sm font-medium h-8 px-5 rounded-sm data-[state=on]:bg-background data-[state=on]:text-foreground data-[state=on]:shadow-sm text-muted-foreground transition-all",
